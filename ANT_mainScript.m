...
%CONSTANTS
ARROW_TAIL_WIDTH= 20; %size of the arrow tail - 0.6cm in the current display
...
MAX_RT = 8.7; %Maximum reaction time (1.7sec)
SECOND_SCREEN_TIME = 0.1; %total time of the cue screen (0.1sec)
THIRD_SCREEN_TIME = 0.4; %total time in secs of the third screen (0.4sec)
black = [0 0 0];
white = [255 255 255];
...
try
    %these variables below store participant data
    participantID = input ('Por favor digite o número do participante: ', 's');
    participantSession = input ('Por favor digite a sessão do participante: ', 's');
    participantName = input ('Por favor digite o nome do participante: ', 's');
    participantSex = input ('Por favor escreva o sexo do participante (f ou m): ', 's');
    numOfTrials = input ('Por favor digite o número de trials que deseja, o normal é 96: ');
    ...
    %Creates a label column for the result matrix
    labels = {'Stimuli'; 'Cue'; 'Response'; 'RT'; 'Accuracy'; ...
    'AuditScreen1'; 'AuditScreen2'; 'AuditScreen3';...
    'AuditScreen4'; 'AuditScreen5'};
    %Concatenate the results matrix with its labels
    resultsMatrix = [labels num2cell(shufledMatrix)];
    
    fileName = ['Participant#' participantID '_Session#' participantSession ...
        '_' participantSex '_' participantName '.csv'];
    %Save the final result matrix to a csv file
    cell2csv(fileName, resultsMatrix, [], 2016, []);
